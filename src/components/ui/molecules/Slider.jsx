      ...
      <Slider {...settings} style={{ height:'100%' }}>
        {
          sliderData.map((e,i) => (
            <Div key={i} bk={{border:'0', display:'grid !important',  gridTemplateColumns:'repeat(6,1fr)', gridTemplateRows:'repeat(6,1fr)', padding:'0', height:'100%', md:{height:'calc(100vh - 100px)'} }}>           
              <GatsbyImage image={getImage(e.node.featuredImage.node)} alt='imagen-slider' css={css`width:100%; height:max(600px,calc(100vh - 100px));  object-fit:cover; grid-column:1/7; grid-row:1/7`}/>
              ...
              <Typography component='a' href={linkWsp} variant={'base'} fw={'title'} bk={{ background:`${theme.palette.primary.main}`,textAlign:'center',padding:'15px 20px',width:'max-content', alignSelf:'center',color:'#fff', margin:'0 auto',gridColumn:'2/6', gridRow:'4/5',color:'#fff' }}>{'Quiero Contactarme'}</Typography>
            </Div>
            ))
          }
