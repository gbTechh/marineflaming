...
export const LayoutScreen = ({ children, title, descripcion }) => {
  ...
  return (
    <>      
      ...
      <Container bk={{ background:'#273358',height:'auto', padding:'0px 30px' }}>
        <Row justify='center' align={'center'} bk={{ height:'100%' }} wrap gap={0}>
          ...
          <Col span={12} bk={{ alignItems:'center', justifyContent:'center', xmd:{width:'40%'} }}>
            <Row justy='center'  bk={{ width:'100%',flexDirection:'column',alignItems:'center', xmd:{flexDirection:'row',justifyContent:'flex-end'} }}>
              <Col span={6} bk={{ width:'100%', justifyContent:'center', alignItems:'flex-end', xmd:{justifyContent:'flex-end'} }}>
                <Div bk={{ width:'40px', height:'40px', border:'0', background:'none', padding:'1px 0' }}>
                  {facebook && <a href={facebook} css={css`width:100%; height:100%; display:flex; align-items:center`}><Facebook css={css`width:100%; height:100%; max-height:100%; fill:#fff`}/></a>}
                </Div>
                <Spacer x={2}/>
                <Div bk={{ width:'40px', height:'40px', border:'0', background:'none', padding:'1px 0' }}>
                {twitter && <a href={twitter} css={css`width:100%; height:100%; display:flex; align-items:center`}><Twitter css={css`width:100%; height:100%; max-height:100%; fill:#fff`}/></a>}
                </Div>
                <Spacer x={2}/>
                <Div bk={{ width:'40px', height:'40px', border:'0', background:'none', padding:'1px 0' }}>
                {instagram && <a href={instagram} css={css`width:100%; height:100%; display:flex; align-items:center`}><Instagram css={css`width:100%; height:100%; max-height:100%; fill:#fff`}/></a>}
                </Div>
                {/* {<Spacer x={2}/>
                <Link to={whatsappLink()} css={css`cursor:pointer;width:130px; height:35px; background:#1cb360; border-radius:99.99px; padding:2px 5px`}><Typography variant='sm' component='span' bk={{ width:'100%', height:'100%', display:'flex', color:'#fff' }}><Whatsapp css={css`margin-right:0px;width:100%; height:100%; max-height:100%; fill:#fff`}/>Whatsapp</Typography></Link>} */}
              </Col>             
            </Row>
          </Col>
        </Row>
