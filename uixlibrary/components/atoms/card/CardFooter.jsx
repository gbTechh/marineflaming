...
const StyledCardFooter = ({ 
  ...
  return {
    ...
    padding: `${px(pd * 1.5)} ${px((pd * 2.4))}`,
    ...(pd ?? {padding: `${px(2 * 1.5)} ${px((2 * 2.4))}`}),
...
const cardFooterConfig = {
  shouldForwardProp: (prop) =>
    isPropValid(prop) && !IGNORED_PROPS.includes(prop),
};


const CardFooterComponent = styled('div', cardFooterConfig)(StyledCardFooter);
